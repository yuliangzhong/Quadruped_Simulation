...
public:
    FootstepPlan()
    {
        terrain = gui::Model(CRL_DATA_FOLDER "/terrain/terrain.obj");
    }
    ...
    Trajectory3D generateLimbTrajectory(RobotLimb* limb,
        ...
        while (t < tEnd) {
            ...
            if (cpi.isStance()) {
                ...
                V3D rayStart = V3D(eePos[0],-10,eePos[2]);
                P3D hitPoint;
                terrain.hitByRay(getP3D(rayStart), V3D(0,1,0),hitPoint);
                groundHeight = hitPoint.y;

                /*
                double del_2 = eePos.x()*eePos.x() + (eePos.z()-5)*(eePos.z()-5);
                if(del_2 <= 9)
                {
                    groundHeight = -8.75 + sqrt(9.25*9.25-del_2);
                }*/
...
private:
    ...
    void generate(const bFrameState& startingbFrameState) {
        ...
        while (t < tEnd) {
            ...
            V3D rayStart = V3D(pos[0],-10,pos[2]);
            P3D hitPoint;
            terrain.hitByRay(getP3D(rayStart), V3D(0,1,0),hitPoint);
            pos.y += hitPoint.y;
            /*
            double del_2 = pos.x*pos.x + (pos.z-5)*(pos.z-5);
            if(del_2 <= 9)
            {
                pos.y += -8.75 + sqrt(9.25*9.25-del_2);
            }*/
            ...
            // My code start---------------------------------------
            V3D Fwd = heading*(robot->forward); // rotate by Quaternion heading
            V3D v1 = vForward*(Fwd)*dt;
            V3D v2 = vSideways*(RBGlobals::worldUp.cross(Fwd))*dt;
            pos = pos + getP3D(v1 + v2);
            headingAngle = headingAngle + turningSpeed*dt;
            // My code end-----------------------------------------
...
public:
    ...
    { 
        this->robot = robot; 
        terrain = gui::Model(CRL_DATA_FOLDER "/terrain/terrain.obj"); //this-> ???
    }
